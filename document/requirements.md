...
2026-01時点での主な追加・変更点：
・全ステップはサイドバーから自由に遷移可能（どこからでも編集・再実行可）
・「選択待ち」ラベルは廃止、ステップ進行の制約なし
・仮説キャンバスまとめ（図UI＋AI自動生成）ステップ追加
・自由記述ステップ追加
・全ステップの入力・AI出力・日時をMarkdown/CSV形式で一括エクスポート可能（右端サイドバー下部に出力枠）
・全ステップAI出力履歴を画面右端サイドバーで常時参照可能（タブ＋カード型UI）
...
- サイドバーから任意の過去ステップに戻り、入力内容を修正・再実行できる
